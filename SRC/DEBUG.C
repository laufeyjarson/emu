...
void DEBUG_INIT(void)
{
    ...
    if(debugWin)
    {
        ...
        {
            SetConsoleTitle("6502 Debugging info");
    ...
    dPrintf("Debugging module started\r\n");
}
...
void DEBUG_MENU_GOT()
{
    ...
    {
        dPrintf("meaningless menu check\r\n");
    }
...
void DEBUG_START_MENU(HWND hWnd)
{
    ...
    if(hMenuDebug == NULL)
    {
        dPrintf("Can't create new menu, none added\r\n");
    }
    AppendMenu(hMenuDebug, MF_STRING|MF_CHECKED, WM_DEBUG_ENABLE, "&Enabled");

    hMenuMain = GetMenu(hWnd);
    if(hMenuMain == NULL)
    {
        dPrintf("No menu in app window - no debug controls added\r\n");
        return;
    }
    AppendMenu(hMenuMain, MF_POPUP, (UINT)hMenuDebug, "&Debug");
...
{
    dPrintf("debugging module stopped\r\n");
#ifdef CONSOLE_DEBUGGER
    FreeConsole();
#endif
...
int dPrintf(char *fmt, ...)
{
    ...
    // dump it out
    OutputDebugString("     EMU6502: ");
    OutputDebugString(pBuff);
#else
    WriteFile(hConsole, "EMU6502: ", 9, &dwWrite, NULL);
    WriteFile(hConsole, pBuff, lstrlen(pBuff), &dwWrite, NULL);
#endif
